<?php
	class Core {
		...
		/**
		  ...
		  * @param $value
		  */
		public function buildField( $type, $required = "", $name, $label, $helper, $value = "", $size = "span3", $placeholder = "" ) {
		
			$build	= "\n<div class=\"control-group\">\n";
			$build .= "<label class=\"control-label\" for=\"{$name}\">{$label}</label>\n";
			...
			if( $type == "text" or $type == "password" ) {
				...
				if( $required ) {
					$build .= "<input type=\"{$type}\" class=\"{$size}\" placeholder=\"{$placeholder}\" name=\"{$name}\" required data-validation-required-message=\"This field is required!\" value=\"{$value}\">";
				}
				else {
					$build .= "<input type=\"{$type}\" class=\"{$size}\" placeholder=\"{$placeholder}\" name=\"{$name}\" value=\"{$value}\">";
				}
				
				if( $helper ) {
					$build .= "<p class=\"help-block\">{$helper}</p>";
				}
			}
			elseif( $type == "textarea" ) {
			
				if( $required ) {
					$build .= "<textarea class=\"input-xlarge\" id=\"{$name}\" rows=\"5\" name=\"{$name}\" required data-validation-required-message=\"This field is required!\">{$value}</textarea>";
				}
				else {
					$build .= "<textarea class=\"input-xlarge\" id=\"{$name}\" rows=\"5\" name=\"{$name}\">{$value}</textarea>";
				}
		...
		/**
		 * buildField - Field builder
		 * @deprecated
		 */
		public function buildField_old( $type, $required, $name, $label, $helper, $value = "" ) {
			...
			elseif( $type == "textarea" ) {
				...
				$build .= "<textarea cols=\"50\" rows=\"5\" id=\"{$name}\" name=\"{$name}\" style=\"width:100%;\">";
				$build .= $value;
				$build .= "</textarea>";
			...
			elseif( $type == "big_textarea" ) {
				...
				$build .= "</td>\n\n<td colspan=\"2\" align=\"right\">\n";
				$build .= "<textarea cols=\"50\" rows=\"10\" id=\"{$name}\" name=\"{$name}\" style=\"width: 100%;\">";
				$build .= $value;
				$build .= "</textarea>";
